...
def test_simulate_dgi_image_matches_pci_dgi_baseline() -> None:
    with np.load(BASELINE_PATH) as baseline:
        metadata = json.loads(str(baseline["metadata_json"]))
        result = simulate_dgi_image(
            baseline["scalar_phase_map_rad"],
            baseline["pupil"],
            stop_optical_depth=metadata["dgi_od"],
        ...
        np.testing.assert_allclose(result["dgi_reference_field"], baseline["dgi_reference_field"], rtol=1e-10, atol=1e-12)
        np.testing.assert_allclose(result["dgi_image_intensity"], baseline["dgi_image_intensity"], rtol=1e-10, atol=1e-12)


def test_simulate_dgi_image_returns_valid_intensity() -> None:
    with np.load(BASELINE_PATH) as baseline:
        metadata = json.loads(str(baseline["metadata_json"]))
        intensity = simulate_dgi_image(
            baseline["scalar_phase_map_rad"],
            baseline["pupil"],
            stop_optical_depth=metadata["dgi_od"],
        )
