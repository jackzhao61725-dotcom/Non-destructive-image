...
def test_canonical_fig_and_table_share_one_parameter_set() -> None:
    ...
    assert all(row["power_mW"] == 1.0 for row in rows)
    assert all(row["exposure_us"] == 90.0 for row in rows)
    assert all(row["QE"] == 0.6 for row in rows)
    assert all(row["read_noise_e_rms"] == 3.0 for row in rows)
...
def test_mislabelled_15us_budget_is_corrected() -> None:
    ...
    assert actual_40["N_stop_heating_continuous"] == pytest.approx(12.4, abs=0.1)
    assert actual_40["N_stop_heating_reabs_continuous"] == pytest.approx(12.0, abs=0.1)
    assert actual_40["recoil_energy_multiplier"] == 2.0
...
def test_route_a_counting_and_reference_values_are_explicit() -> None:
    audit = _audit()
    route = next(row for row in audit["budget_rows"] if row["parameter_set"] == "canonical_route_a")
    along = {row["Delta_over_2pi_GHz"]: row for row in audit["along_cigar_rows"]}

    assert route["N_max_clean_continuous"] == pytest.approx(46.01634460130998, rel=1e-12)
    assert route["N_stop_heating_reabs_continuous"] == pytest.approx(10.70033225380004, rel=1e-12)
    assert route["strict_integer_clean_frames"] == 46
    assert route["strict_integer_heating_reabs_frames"] == 10
...
def test_full_model_reference_uses_same_route_a_but_distinct_observable() -> None:
    ...
    assert all(int(float(row["n_frames_full"])) == 10 for row in rows)
    assert all(row["observable"] == "fixed-ROI diagonal-covariance matched-filter SNR" for row in rows)
    assert values[("PCI", "shot_plus_read_noise")] == pytest.approx(103.668651367624, rel=1e-12)
    assert values[("DGI", "shot_plus_read_noise")] == pytest.approx(34.30325710456135, rel=1e-12)
...
def test_operating_point_snr_is_recomputed_with_explicit_40us() -> None:
    ...
    assert across["realistic_peak_pixel_SNR"] == pytest.approx(5.781195504124932, rel=1e-12)
    assert across["realistic_resolution_element_SNR"] == pytest.approx(11.691905893082325, rel=1e-12)
    assert along["ideal_QE1_peak_pixel_SNR"] == pytest.approx(36.45263879372922, rel=1e-12)
    assert along["realistic_peak_pixel_SNR"] == pytest.approx(6.0149162768925635, rel=1e-12)
    assert along["realistic_resolution_element_SNR"] == pytest.approx(8.525022820339919, rel=1e-12)
