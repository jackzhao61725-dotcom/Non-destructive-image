...
def write_camera_figure(path: Path, config: dict[str, Any], stage: dict[str, Any]) -> None:
    ...
    ax_map.set_title("deterministic camera image, no noise")
    plt.colorbar(im, ax=ax_map, fraction=0.03, label="$I/I_0$")

    ax_line.plot(binned_axis_y_um, image[mid_row, :], "C1", lw=2, label="camera cut along y")
    ax_line.plot(binned_axis_z_um, image[:, mid_col], "C0", lw=2, label="camera cut along z")
    ax_line.set_xlim(-45, 45)
    ax_line.set_xlabel("position (um)")
    ax_line.set_ylabel("$I/I_0$")
    ax_line.set_title("centre cuts after 15 x 15 binning")
    ax_line.legend(fontsize=8.5)
    ax_line.grid(alpha=0.25)
    fig.suptitle("Notebook-aligned camera recovery: ideal PCI image -> deterministic camera image", fontsize=12)
    plt.tight_layout()
